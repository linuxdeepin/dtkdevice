...
static string sysfstobusinfo(const string &path)
{
    ...
    if (bustype == "ccwgroup") {
        ...
        // because the group doesn't really fit into lsdevice's tree model
...
{
    string product = get_string(This->devpath + "/device");
    if (product.empty())
        product = get_string(This->devpath + "/product");  //usb
    return  product;
}

string entry::vendor() const
{
    string vendor = get_string(This->devpath + "/vendor");
    if (vendor.empty())
        vendor = get_string(This->devpath + "/manufacturer");  //usb
    return  vendor;
}

string entry::sysfs_path() const
{
    return (This->devpath);
}
